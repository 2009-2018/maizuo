<template>
  <div class="willplay">
    ...
    <div class="hot_item" v-for="item, index in filmArr" @click="go(item)">
    ...
    methods: {
      ...
      async getMoreInit(str){
        ...
        if(!str){ //如果没穿具体要请求的 电影id
          ...
          data = await this.$http.get('/maoyan/ajax/comingList?ci=60&token=&limit=10');
          //储存影片id信息
          this.movieIdsArr = data.data.movieIds; //储存 正确的影片信息数据
          // console.log(this.movieIdsArr);

        }else {
          data = await this.$http.get('/maoyan/ajax/moreComingList?ci=60&token=&limit=10&movieIds='+str);
